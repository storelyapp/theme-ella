class CollectionFiltersForm extends HTMLElement {
  constructor() {
    ...
    this.sidebarBlocks = this.querySelectorAll(
      ".collection-filters__item [data-type-list]"
    );
    ...
    if (this.querySelector("#filter__price--apply")) {
      this.querySelector("#filter__price--apply").addEventListener(
        "click",
        this.debouncedOnClick.bind(this)
      );
    }
    ...
    if (filterDisplayType === "show-more")
      CollectionFiltersForm.initListShowMore(this.sidebarBlocks);
    ...
    sessionStorage.setItem(
      "productGridId",
      JSON.stringify(
        document.getElementById("main-collection-product-grid").dataset.id
      )
    );
  ...
  static initListShowMore(sidebarBlocks) {
    ...
    sidebarBlocks.forEach((block, blockIndex) => {
      ...
      if (currentHeight === 0) {
        const contentWrapper = facetList?.closest(
      ...
      storedExpandedAndCollapsedHeight.push({
      ...
      if (blockIndex === sidebarBlocks.length - 1) {
        sessionStorage.setItem(
          "heightData",
          JSON.stringify(storedExpandedAndCollapsedHeight)
        );
      }
    });
    ...
    if (showMoreButtons) {
      showMoreButtons.forEach((button) =>
        button.addEventListener("click", CollectionFiltersForm.toggleShowMore)
      );
    }
  ...
  static toggleShowMore(event) {
    let storedExpandedAndCollapsedHeight =
      JSON.parse(sessionStorage.getItem("heightData")) || [];

    const showMoreButton = event.target.closest("[data-show-more-btn]");
    const showMoreButtonContent = showMoreButton.querySelector(
    ...
    const facetListItems = sideBlockContent.querySelectorAll(
      ".facets__list .list-menu__item"
    );
    const storedBlockData = storedExpandedAndCollapsedHeight.find(
      (item) => item.index === sideBlock.dataset.index
    );
    const collapsedHeight =
      parseInt(sideBlockContent.dataset.collapsedHeight) ||
      storedBlockData.collapsedHeight;
    ...
    if (maxItemsNumber > 10 && maxItemsNumber <= 20)
      maxExpandedHeight = collapsedHeight * 2;

    if (maxExpandedHeight > window.innerHeight)
      maxExpandedHeight = collapsedHeight * 1;
  ...
  static renderRemainingFilters(sidebarBlocks) {
    ...
    sidebarBlocks.forEach((element) => {
      ...
      const currentSideblockData = storedExpandedAndCollapsedHeight?.find(
        (item) => item.index === element.dataset.index
      );
      ...
      const showMoreContent = showMoreButton?.querySelector(
        "[data-show-more-content]"
      );
      ...
      showMoreButton?.addEventListener(
        "click",
        CollectionFiltersForm.toggleShowMore
      );
  ...
  static renderPriceFilter(priceFilterBlock) {
    if (priceFilterBlock) {
      const htmlElement = document.querySelector(
        `.js-filter[data-index="${priceFilterBlock.dataset.index}"]`
      );
      if (htmlElement == null) return;
      ...
      if (htmlElement.querySelector("#filter__price--apply")) {
        htmlElement
          .querySelector("#filter__price--apply")
          .addEventListener("click", debouncedOnClick);
      }
    }
  }
  ...
  static setListeners() {
    const onHistoryChange = (event) => {
      const searchParams = event.state
        ...
        : CollectionFiltersForm.searchParamsInitial;
  ...
  static resizeGridItem(item) {
    ...
    rowSpan = Math.ceil(
      (item
        .querySelector(
          ".collection-masonry .product-masonry-item .product-item"
        )
  ...
  static renderPage(searchParams, event, updateURLHash = true) {
    ...
    sections.forEach((section) => {
      ...
      CollectionFiltersForm.filterData.some(filterDataUrl)
        ? CollectionFiltersForm.renderSectionFromCache(
            filterDataUrl,
            section,
            event
          )
  ...
  static setProductForWishlist(handle) {
    ...
    if (index >= 0) {
      item.addClass("wishlist-added").find(".text").text(window.wishlist.added);
    } else {
      item
  ...
  static renderProductGrid(html) {
    ...
    document
      .getElementById("CollectionProductGrid")
      .querySelector(".collection").innerHTML = innerHTML;
    ...
    if (document.querySelector(".collection-masonry")) {
      document
        .getElementById("CollectionProductGrid")
        .querySelector(".collection .halo-row--masonry")
  ...
  static renderFilters(html, event) {
    ...
    Array.from(facetDetailsElementsFromDom).forEach((currentElement) => {
      const existsInFetched = Array.from(facetDetailsElements).some(
        ({ dataset }) => currentElement.dataset.index === dataset.index
      );
      if (!existsInFetched) {
    ...
    if (document.querySelector(`.facets__reset[data-index="${indexTarget}"]`)) {
      document.querySelector(
        `.facets__reset[data-index="${indexTarget}"]`
      ).style.display = "block";
    }
    ...
    if (countsToRender)
      CollectionFiltersForm.renderCounts(
        countsToRender,
        event.target.closest(".js-filter")
      );

    if (sessionStorage.getItem("filterDisplayType") === "show-more") {
      const remainingSidebarBlocks = Array.from(facetDetailsElements).filter(
        (element) =>
          element.dataset.typeList && element.dataset.index !== indexTarget
      );
      CollectionFiltersForm.renderRemainingFilters(remainingSidebarBlocks);
    }

    CollectionFiltersForm.renderPriceFilter(
      Array.from(facetDetailsElements).find(
        (element) =>
          element.dataset.typePrice && element.dataset.index !== indexTarget
      )
    );
  }
  ...
  static renderActiveFacets(html) {
    ...
    setTimeout(() => {
      const sidebarBlocks = html.querySelectorAll(
        ".collection-filters__item [data-type-list]"
      );
    ...
    return [
      {
        ...
        section:
          document.getElementById("main-collection-product-grid")?.dataset.id ||
          JSON.parse(sessionStorage.getItem("productGridId")),
      },
    ];
  }
  ...
  static initViewModeLayout(column) {
    const productListing = document
      .getElementById("CollectionProductGrid")
      .querySelector(".productListing");

    if (!productListing) return;

    switch (column) {
      case 1:
        productListing.classList.remove(
          "productGrid",
          "column-5",
          "column-4",
          "column-3",
        ...
        $searchParams.forEach((element) => {
          ...
          switch (attr) {
            case "filter.v.price.gte":
              ...
              params == ""
                ? (params = `filter.v.price.gte=${min}`)
                : (params = `${params}&filter.v.price.gte=${min}`);
              break;
            case "filter.v.price.lte":
              let max = Math.round(val * ratio);
              params == ""
                ? (params = `filter.v.price.lte=${max}`)
                : (params = `${params}&filter.v.price.lte=${max}`);
              break;
            default:
              params == ""
                ? (params = element)
...
class PriceRange extends HTMLElement {
  ...
  rangeSliderPrice() {
    let rangeS = this.querySelectorAll("input[type=range]"),
      ...
      slide =
        (this.querySelector("input[type=range]").max / 100) *
        (22 / (this.offsetWidth / 100));
  ...
  updateDisplay(value1, value2) {
    ...
    const priceSlideRangeContainer = this.querySelector(
      ".facets__price--slide"
    );
    const max = range.max;
